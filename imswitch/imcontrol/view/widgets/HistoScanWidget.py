...
class Canvas(QtWidgets.QLabel):
    ...
    def overlayImage(self, npImage, alpha=0.5):
        ...
        channel = npImage[:,:,0].copy()
        height, width, channel = npImage.shape
        bytesPerLine = width
        qImage = QImage(npImage.data, width, height, QImage.Format_Mono)
        
        self.setPixmap(self.imageNow.scaled(self.dimensions[0], self.dimensions[1]))
    ...
    def setImage(self, npImage=None, pathImage='histo.jpg'):
        ...
        if len(npImage.shape) == 2:
            npImage = np.repeat(npImage[:,:,np.newaxis], 3, axis=2)
        self.imageLast=npImage.copy() # store for undo
        height, width, channel = npImage.shape
        bytesPerLine = 3 * width

        qImage = QImage(npImage.copy(), width, height, QImage.Format_RGB888)
