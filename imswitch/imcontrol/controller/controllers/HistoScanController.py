...
class HistoScanController(LiveUpdatedController):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._logger.debug("allDetectorNames: {}".format(allDetectorNames))
        self.detector = self._master.detectorsManager[allDetectorNames[0]]
        self._camera = self.detector
    ...
    def startHistoScan(self):
        ...
        self.HistoScanDate = datetime.now().strftime("%Y_%m_%d-%I")
    ...
    def doZickzackScan(self, overlap):
        # TODO: START AND END ON COORDINATES GIVEN BY USER
        initPos = (14764,8743,-307)
        nPixels = (3072,2048)
        pixelSize = self.detector.pixelSizeUm
        self._logger.debug("pixelSize: {}".format(pixelSize))
        ...
        filePath = self.getSaveFilePath(date=self.HistoScanDate,
                                        filename=f'{self.HistoScanFilename}',
                                        extension='ome.tif')
        with tif.TiffWriter(filePath, bigtiff=True) as tiff:
            for ix in range(nX):
                for iy in range(nY):
                    posX = ix * pixelSize[1]*nPixels[0]*overlap+initPos[0]
                    posY = iy * pixelSize[2]*nPixels[1]*overlap+initPos[1]
                    ...
                    tiff.write(lastFrame, metadata=metadata)

                # api.imcontrol.movePositioner(positionerName, "XY", (posX,posY), True, True)
                self._logger.debug("Moving to position: {}".format((posX, posY)))
        self._logger.debug("Finished scanning at: {}".format(datetime.now().strftime("%Y_%m_%d-%I-%M-%S")))
        # return coordinates

    def doScan(self):
        # TODO: initailize pixelsize (low-res and highres) and stage stepsize

        # 1. Move to initial position on sample
        # TODO: RETRIEVE PROPER COORDINATES THROUGH GUI AND SETTINGS
        initial_coordinates = (self.currentPositionX, self.currentPositionY)
        self._logger.debug("Initial coordinates: {}".format(initial_coordinates))

        # 2. Take low-res, arge FOv image
        if len(self._widget.canvas.getCoordinateList()) <= 0:
            # TODO: ACTUALLY TAKE LOW-RES IMAGE
            self._logger.debug("No selection was made..")

        overlap = 0.3   # %
        self.isHistoScanrunning = True
        self.HistoScanThread = threading.Thread(target=self.doZickzackScan, args=(overlap,), daemon=True)
        self.HistoScanThread.start()

        # 3. Get Annotaitons from sample selection and bring them to real world coordinates
        # allCoordinatesToScan = np.array(self._widget.canvas.getCoordinateList())

        # translate coordinates into bitmap coordinates
        # scanRegion = np.zeros((self.camPreviewPixNumX, self.camPreviewPixNumY))
        # scanRegion[allCoordinatesToScan[:, 0], allCoordinatesToScan[:, 1]] = 1
        ...
        # self.HistoScanThread = threading.Thread(target=self.doScanThread, args=(allCoordinatesToScan,), daemon=True)
        # self.HistoScanThread.start()
    ...
    def doScanThread(self, coordinateList):
        ...
        initialPosition = (coordinateList[0])
        ...
        for iPos in range(len(coordinateList)):
            ...
            # TODO: ensure it's the right light source!
        ...
        self.stages.move(value=initialPosition, axis="XY", speed=(self.speed, self.speed),
                         is_absolute=True, is_blocking=True, timeout=5)
    ...
    def snapPreview(self):
        ...
        self._logger.debug("Preview image shape: " + str(self.previewImage.shape))
        self._logger.debug(self.detector.name)
        self._logger.debug("Frame: " + str(self.previewImage))
